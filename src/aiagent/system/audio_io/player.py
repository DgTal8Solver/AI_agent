...
class AudioPlayer:
    ...
    def _callback(
        ...
        try:
            chunk = self._processed_queue.get_nowait()
            data[:] = chunk[:frames]
        except queue.Empty:
            data.fill(0)
            RuntimeWarning("Underflow: have not enough data!")
    ...
    def start(self) -> None:
        if self._running:
            raise RuntimeError("AudioPlayer is already running!")
        ...
        self._running = True
        print(f"AudioPlayer started (blocksize = {self._blocksize})")
